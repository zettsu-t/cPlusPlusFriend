...
$$
Var[x] = \mathbb{E}[x] + Var[Gamma(\alpha_{gamma}, \beta_{gamma}] = \frac {\alpha_{gamma}}{\beta_{gamma}} (1 + \frac{1} {\beta_{gamma}}) = \mathbb{E}[x] (1 + \frac{1} {\beta_{gamma}}) \tag{`r EqnNum(head)`} \label{eq:gamma_poisson_x2}
$$
...
$$
\mathbb{E}[y] = size \frac {1-prob}{prob}, Var[y]= size \frac {1-prob}{{prob}^2} = \frac {\mathbb{E}[y]}{prob} \tag{`r EqnNum(head)`} \label{eq:nb_y_size_prob}
$$

であり、平均と分散が既知なら(観測値があれば)、負の二項分布のパラメータ$size, prob$を求めることができる。

$$
prob = \frac {\mathbb{E}[y]}{Var[y]}, size = Var[y] * \frac {{prob}^2}{1-prob} = \frac {{\mathbb{E}[y]}^2}{Var[y] - \mathbb{E}[y]} \tag{`r EqnNum(head)`} \label{eq:nb_size_prob}
$$
...
$$
\beta_{gamma} = \frac {Var(x)}{\mathbb{E}(x)} = \frac {Var(y) - \mathbb{E}(x)}{\mathbb{E}(x)} = \frac {Var(y) - \mathbb{E}(y)}{\mathbb{E}(y)} = \frac {1}{p} - 1 \tag{`r EqnNum(head)`} \label{eq:nb_alpha_beta_xy1}
$$

$$
\alpha_{gamma} = \frac {\mathbb{E}[x]}{\beta} = \frac {\mathbb{E}[y]}{\beta} = size \tag{`r EqnNum(head)`} \label{eq:nb_alpha_beta_xy2}
$$

つまり、$\alpha_{gamma}=size, \beta_{gamma}=1/p - 1$が成り立つ。これは\ref{eq:stan_nb}と\ref{eq:dnbinom}の関係であった。
...
$$
NB(y|size_A+size_B, prob) = \sum_{i=0}^y {\frac {\Gamma(i+size_A)}{\Gamma(size_A) i!} p^{size_A} (1-p)^i \frac {\Gamma(y-i+size_B)}{\Gamma(size_B) (y-i)!} p^{size_B} (1-p)^{y-i} } \tag{`r EqnNum(head)`} \label{eq:nb_sum_proof1}
$$

$$
= p^{size_A + size_B} (1-p)^y \sum_{i=0}^y { \frac {\Gamma(i+size_A)}{\Gamma(size_A) i!} \frac {\Gamma(y-i+size)}{\Gamma(size) (y-i)!} } \tag{`r EqnNum(head)`} \label{eq:nb_sum_proof2}
$$
...
である。これはn人がそれぞれsize回当たるまでくじを引く($NB(size, prob)$)ときの合計試行回数と、一人でsize*n回当たるまでくじを引く($NB(size*n, prob)$)ときの試行回数が、同じ分布に従うということである。n人が同一人物と考えれば、そうなるのが自然である。

```{r nb_scale_alpha, fig.height=4, fig.width=8, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:figure1}negative binomial(size=16.0, prob=0.4) alpha combined"}
draw_distributions(n_sample=10000, nb_size=16.0, nb_prob=0.4, divider=16, is_size_divided=TRUE, is_nb=TRUE)
```

図\ref{fig:figure1}はsize=16, prob=0.4の負の二項分布に基づいて生成した乱数のヒストグラムである。横軸は$x$、縦軸は各$x$の出現回数である。Expectedは\ref{eq:nb_size_prob}による平均と分散、oneは単一の負の二項分布、combinedはsizeを16分割した複数の負の二項分布の和である。

\ref{eq:gamma_mu_var}より、ガンマ分布の期待値は$\alpha:=shape$にも$\theta:=scale=1/\beta$にも比例するが、\ref{eq:nb_mu_var}よりガンマ-ポアソン分布の期待値と分散は$\alpha$に比例しても、分散は$\theta=1/\beta$に比例しない。なので、$\theta$をn分割した負の二項分布の和は、元の負の二項分布とは異なる(図\ref{fig:figure2})。

```{r nb_scale_theta, fig.height=4, fig.width=8, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:figure2}negative binomial(size=16.0, prob=0.4) theta combined"}
draw_distributions(n_sample=10000, nb_size=16.0, nb_prob=0.4, divider=16, is_size_divided=FALSE, is_nb=TRUE)
```
...
```{r gamma_scale_alpha, fig.height=4, fig.width=8, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:figure1}Gamma distribution alpha combined"}
draw_distributions(n_sample=10000, nb_size=16.0, nb_prob=0.4, divider=16, is_size_divided=TRUE, is_nb=FALSE)
```

やはり負の二項分布と同様、$\theta$の分割は再生性がない(期待値は同じだが分散は異なる)。

```{r gamma_scale_theta, fig.height=4, fig.width=8, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:figure2}Gamma distribution theta combined"}
draw_distributions(n_sample=10000, nb_size=16.0, nb_prob=0.4, divider=16, is_size_divided=FALSE, is_nb=FALSE)
```
