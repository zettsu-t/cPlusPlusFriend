...
Rスクリプトで作成したPNG画像を、一枚一枚文書に貼り込むのは辛いのです。プログラマの三大美徳(短気、怠惰、傲慢)があれば実行可能ファイルをmake一発できるなら、文書作成もなんとかならないでしょうか。

"再現可能性のすゝめ (Wonderful R 3)" (高橋康介 著/石田基広 監修, 2018/5, 共立出版)を読んで、R Markdownを使えばレポートが作成できることが分かったので、試してみましょう。

## 例題

[学校保健統計調による17歳の身長](https://www.e-stat.go.jp/stat-search/files?page=1&layout=datalist&toukei=00400002&tstat=000001011648&cycle=0&tclass1=000001113655&tclass2=000001113656&second2=1) をプロットしてみます。
...
g <- g + xlab('Height [cm]')
g <- g + ylab('Percentage')
g <- g + labs(title='Heights of 17-year-old')
plot(g)
```
...
このように標本をカテゴリ=ここでは性別を区別すると、それぞれをよく知られた分布で表現できそうです。試しに、正規分布かどうかQQ normで描いてみましょう。

```{r qqnorm_all, fig.height=5, fig.width=8, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE}
par(mfrow=c(1,3))
