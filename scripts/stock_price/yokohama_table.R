...
to_color_table <- function(df) {
  df %>%
    ...
    purrr::reduce(.x=tail(seq_len(NCOL(df)), -3), .init=., function(acc, col) {
      offset <- 1
      values <- as.vector(unlist(df_population_data[-offset, col]))
      ward_max <- offset + which.max(values)
      ward_min <- offset + which.min(values)
