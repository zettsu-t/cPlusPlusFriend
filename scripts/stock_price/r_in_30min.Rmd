...
R MarkdownをHTML化したときの引数が出ていますが、 R Markdown ファイルの場所がフルパスで見えてしまうのは削除した。
...
横浜市の合計を各区を分離しよう。 dplyr::filter を使って、～区とそれ以外を抜き出す。str_endは文字列がある文字列で終わっているかどうかをTRUEまたはFALSEで返す。Pythonの endswith と同じである。
...
ちなみに dplyr::mutate の代入先もシンボルであり、実行時に文字列を与えるときは rlang::syms を使う。このときは = ではなく := を使う。代入元がシンボルの場合も同様である。
...
- 変数名に対する指摘は、仕方ない場合もある。指摘に対応しないこともしばしばまにある。
