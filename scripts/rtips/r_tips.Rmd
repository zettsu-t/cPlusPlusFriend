...
「Pythonプログラマが30分でわかるR」では、関数合成に `functional::Compose` を使っていた。これはこれでよいが同様なことを、 `purrr::compose` を使ってできる。Tidyverse をインポートすれば使えるので、こちらの方が便利だろう。`purrr::compose` の関数の適用順序は、 `functional::Compose` とは逆に右から左である(`.dir = forward` 引数で左から右にできる)。
